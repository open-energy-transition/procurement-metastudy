...
2. **24/7 Carbon-Free Energy**: strategy that seeks to match electricity consumption with CFE on an hourly basis. Different matching levels can be considered (i.e., 100% or less). Then, it includes locally procured assets and import from the outer grid, in case the matching level is lower than 100%. The implementation of the 24/7 CFE strategy in this project was based on the [PyPSA code for exploring the 24/7 Carbon-Free Energy procurement](https://github.com/PyPSA/247-cfe).
...
The implementation of the Emission Matching strategy in this project was based on [WattTime](https://watttime.org/)'s constraint definition and data.

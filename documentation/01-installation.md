...
The repository consists of two additional branches outside of the `master` branch which contains the main code base for the study: `gh-pages`, which is necessary for rebuilding this documentation website, and `scenario/`, which is the archives of past runs

Then, clone your newly created repository using the version control system `git` in the command line.

```
git clone https://github.com/<your-username>/procurement-metastudy
```
...
If you want to try out the model as is, you can clone OETs version of [Procurement Metastudy PyPSA-Eur repository](https://github.com/open-energy-transition/procurement-metastudy) instead. 
    
```
git clone https://github.com/open-energy-transition/procurement-metastudy
```
